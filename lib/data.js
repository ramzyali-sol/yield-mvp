...
/* ─── BORROW-SIDE MARKET IMPACT MODEL ───────────────────────────────────── */
export function computeBorrowImpact(borrowAmount, reserveTvl, supplyApy, borrowApy) {
  if (!reserveTvl || reserveTvl <= 0 || !borrowApy || borrowApy <= 0)
    return { effectiveRate: borrowApy || 0, impactPct: 0, estimatedUtil: 0 };
  const estimatedUtil = Math.min(Math.max(
    supplyApy > 0 ? supplyApy / (borrowApy * 0.85) : 0.5, 0.1), 0.95);
  const availableLiquidity = reserveTvl * (1 - estimatedUtil);
  if (availableLiquidity <= 0)
    return { effectiveRate: borrowApy * 2, impactPct: 100, estimatedUtil };
  const borrowImpact = borrowAmount / (availableLiquidity + borrowAmount);
  const kinkMultiplier = estimatedUtil > 0.8 ? 3 : estimatedUtil > 0.6 ? 1.5 : 1;
  const effectiveRate = borrowApy * (1 + borrowImpact * kinkMultiplier);
  const impactPct = ((effectiveRate - borrowApy) / borrowApy) * 100;
  return { effectiveRate, impactPct, estimatedUtil, availableLiquidity };
}
...
export function computeStructuredPosition({
  ...
  const borrowReserveTvl = borrowMarket?.reserve?.tvl || 0;
  const borrowSupplyApy = borrowMarket?.reserve?.supplyApy || 0;
  const { effectiveRate: effectiveBorrowRate, impactPct: borrowImpactPct, estimatedUtil }
  ...
  return {
    ...
    actualLTV, borrowUSD,
    baseBorrowRate, effectiveBorrowRate, borrowImpactPct, estimatedUtil,

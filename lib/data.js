...
// The API borrow rate already reflects current utilization. We only model
// marginal impact when the new borrow is a significant % of total pool supply.
export function computeBorrowImpact(borrowAmount, reserveTvl, supplyApy, borrowApy) {
  ...
  const fractionOfPool = borrowAmount / reserveTvl;
  // Below 5% of pool supply — negligible impact, use raw rate
  if (fractionOfPool < 0.05)
    return { effectiveRate: borrowApy, impactPct: 0 };
  // Above 5%: simple linear impact proportional to pool fraction
  const effectiveRate = borrowApy * (1 + fractionOfPool);
  const impactPct = fractionOfPool * 100;
  return { effectiveRate, impactPct };
}

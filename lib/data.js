...
const TOKEN_LOGOS = {
  ...
  ONYC:    "https://www.google.com/s2/favicons?domain=onre.fi&sz=64",
...
export const VENUES = [
  { name:"Jupiter Lend",        logo:"JL", logoUrl:venueFavicon("jup.ag"),             color:"#C8A84B", category:"lending",    type:"Lending Market",         stableApy:null, solApy:null, tvl:null, audits:["OtterSec","Sec3"],           oss:true,  risk:"LOW",    note:"High LTV, Jupiter DEX integration.", url:"https://jup.ag/lend" },
  ...
  { name:"Save (Solend)",       logo:"Sv", logoUrl:venueFavicon("save.finance"),       color:"#00C8FF", category:"lending",    type:"Lending Market",         stableApy:null, solApy:null, tvl:null, audits:["Kudelski","OtterSec"],       oss:true,  risk:"LOW",    note:"OG Solana lending since 2021. Longest battle-tested track record.", url:"https://save.finance" },
  { name:"Loopscale",           logo:"L",  logoUrl:venueFavicon("loopscale.com"),      color:"#A78BFA", category:"lending",    type:"Fixed-Rate Order Book",  stableApy:null, solApy:null, tvl:null, audits:["Halborn"],                   oss:false, risk:"MEDIUM", note:"Fixed-rate fixed-term order book. No rate volatility. Oracle hack Apr 2025 — funds recovered.", flag:"⚠ Prior exploit — model upgraded post-incident", url:"https://loopscale.com" },
  { name:"Drift Vaults",        logo:"D",  logoUrl:venueFavicon("app.drift.trade"),    color:"#FF6B35", category:"vault",      type:"Earn Vault",             stableApy:null, solApy:null, tvl:null, audits:["OtterSec","Trail of Bits"],  oss:true,  risk:"LOW",    note:"Passive vault earning from Drift insurance fund + lending. Stable rate profile.", url:"https://app.drift.trade/earn" },
  ...
  { name:"OnRe",                logo:"Or", logoUrl:venueFavicon("onre.fi"),            color:"#F59E0B", category:"rwa",        type:"Reinsurance RWA",        stableApy:null, solApy:null, tvl:null, audits:["OtterSec"],                  oss:false, risk:"LOW",    note:"ONyc backed by Bermuda-licensed reinsurance. 9–15% base APY uncorrelated to crypto. Dual-regulated.", url:"https://onre.fi" },
...
export function computeCarryTrade(collateral, amount, ltvPct, venue) {
  ...
  // Apply market impact if venue has TVL data
  const { impactFactor, impactPct } = computeMarketImpact(borrowUSD, venue.tvl);
...
export function enrichVenues(liveData) {
  ...
  for (const km of kaminoMarkets) {
    ...
    kaminoVenues.push({
      name: km.name,
      logo: "K",
      logoUrl: venueFavicon("kamino.finance"),
      color: "#14F195",
      category: "lending",

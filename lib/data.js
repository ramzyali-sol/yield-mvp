...
export const VENUES = [
  ...
  /* Loopscale and Drift are now dynamically created from live API data (like Kamino) */
...
export function enrichVenues(liveData) {
  ...
  // 2. Build dynamic Drift vault entries
  const driftVenues = [];
  const driftVaults = liveData.driftVaults || [];

  for (const dv of driftVaults) {
    ...
    driftVenues.push({
      name: dv.name,
      logo: "D",
      logoUrl: venueFavicon("app.drift.trade"),
      color: "#FF6B35",
      category: "vault",
      type: `${dv.symbol} Earn Vault`,
      stableApy: live.stableApy,
      solApy: live.solApy,
      tvl: live.tvl,
  ...
  driftVenues.sort((a, b) => (b.tvl || 0) - (a.tvl || 0));

  // 3. Build dynamic Loopscale market entries
  const loopscaleVenues = [];
  const loopscaleMarkets = liveData.loopscaleMarkets || [];

  for (const lm of loopscaleMarkets) {
    ...
    loopscaleVenues.push({
      name: lm.name,
      logo: "L",
      logoUrl: venueFavicon("loopscale.com"),
      color: "#A78BFA",
      category: "lending",
  ...
  // 4. Enrich static (non-dynamic) venues with live data
  ...
  // 5. Combine: dynamic venues first (by TVL), then static venues
  return [...kaminoVenues, ...driftVenues, ...loopscaleVenues, ...enrichedStatic];
}

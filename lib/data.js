...
export function getVenuesForAsset(asset, venues) {
  ...
  return v.filter(x => {
    ...
    if (x.reserves && Object.keys(x.reserves).length > 0) {
      ...
      // For same-type matching: e.g., USDC user should see venues with USDT reserves too?
      // No — be strict. Only show venues that specifically list this asset.
    ...
    if (asset.type === "btc")    return x.solApy != null; // BTC venues tend to also have SOL
    return x.stableApy != null || x.solApy != null; // Show if venue has any yield
  });
}
...
export function enrichVenues(liveData) {
  ...
  for (const sv of (liveData.driftStrategyVaults || [])) {
    ...
    driftStratVenues.push({
      ...
      risk: "MEDIUM",
      noImpact: true,
      note: `Drift strategy vault: ${sv.vaultName}. Actively managed.`,
  ...
  // Sort strategy vaults by 30d APY descending
  driftStratVenues.sort((a, b) => {
    const aApy = Math.max(a.stableApy || 0, a.solApy || 0);
    const bApy = Math.max(b.stableApy || 0, b.solApy || 0);
    return bApy - aApy;
  });

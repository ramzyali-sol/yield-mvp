...
export function findBestBorrowMarket(borrowAsset, venues, collateral) {
  // Require minimum $50k TVL in the borrow reserve
  const MIN_BORROW_TVL = 50_000;
  let best = null;
  for (const v of venues) {
    const r = v.reserves?.[borrowAsset.symbol];
    if (!r || !r.borrowApy || r.borrowApy <= 0) continue;
    if ((r.tvl || 0) < MIN_BORROW_TVL) continue;
    // Venue must also support the collateral asset — you can only borrow
    // from a market where you can deposit your collateral
    if (collateral && v.reserves && Object.keys(v.reserves).length > 0) {
      if (!v.reserves[collateral.symbol]) continue;
    }
...
export function getRelevantApy(venue, asset) {
  ...
  // If venue has reserve-level data for this specific asset, use the asset-specific
  // rate — even if it's 0%. Falling through to stableApy would use a different
  // token's rate (e.g. USDT rate for a USDC deploy) which is misleading.
  if (venue.reserves?.[asset.symbol] != null) {
    return venue.reserves[asset.symbol].supplyApy || 0;
  }
